...
/* harmony import */ var _modules_video__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modules/video */ "./js/modules/video.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('load', function () {
    Object(_modules_video__WEBPACK_IMPORTED_MODULE_3__["initVideos"])();
    Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_1__["initTabs"])();
...
var staffSliderElement = document.querySelector('.staff__slider');
function addTabindex() {
  var originalSlides = document.querySelectorAll('.staff__item:not(.swiper-slide-duplicate)');
...
var initSliders = function initSliders() {
          ...
          document.addEventListener('keydown', function (evt) {
            if (evt.shiftKey && evt.key === 'Tab') {
              ...
              if (active.parentElement.classList.contains('swiper-slide-active')) {
                var parentSlideIndex = active.parentElement.dataset.swiperSlideIndex;
              ...
              if (_active.parentElement.classList.contains('swiper-slide-next')) {
  ...
  var notificationSpans = document.querySelectorAll('.swiper-notification');
  if (notificationSpans && notificationSpans.length) {
    notificationSpans.forEach(function (span) {
      span.remove();
    });
  }
};
...
var tabsParentElement = document.querySelector('[data-tabs="parent"]');
var controlClickHandler = function controlClickHandler(evt, controls, elements, parent) {
  var target = evt.target;
  var dataContent = target.dataset.content;
  var correspondingTab = parent.querySelector("[data-element-content=\"".concat(dataContent, "\"]"));
  if (!correspondingTab) {
...
var initTabs = function initTabs() {
  if (tabsParentElement) {
    var parentHeight = tabsParentElement.offsetHeight;
    tabsParentElement.setAttribute('style', "min-height: ".concat(parentHeight, "px"));
    var tabsControls = tabsParentElement.querySelectorAll('[data-tabs="control"]');
    var tabsElementsList = tabsParentElement.querySelectorAll('[data-tabs="element"]');
    if (tabsControls && tabsControls.length && tabsElementsList && tabsElementsList.length) {
      tabsControls.forEach(function (control) {
        var dataContent = control.dataset.content;
        var correspondingTab = tabsParentElement.querySelector("[data-element-content=\"".concat(dataContent, "\"]"));
        if (!correspondingTab) {
...
var videoBlocks = document.querySelectorAll('[data-video-block]');
var createIframe = function createIframe(block) {
...
var initVideos = function initVideos() {
  if (videoBlocks && videoBlocks.length) {
    videoBlocks.forEach(function (block) {
      var videoCover = block.querySelector('[data-video-cover]');
      var videoButton = block.querySelector('[data-video-button]');
      var iframeBlock = block.querySelector('[data-video-container]');
      videoButton.addEventListener('click', function () {
        var newIframe = createIframe(iframeBlock);
